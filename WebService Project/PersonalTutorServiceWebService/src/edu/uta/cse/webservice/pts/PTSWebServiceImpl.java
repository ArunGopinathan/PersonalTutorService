...
public class PTSWebServiceImpl {
	...
	public String basicSearch(@PathParam("query") String query) {
		String result = "HELLO";
		
		MySqlHelper helper = new MySqlHelper();
		String sqlquery = "select * from login l inner join service s on l.UserId = s.UserId inner join personalinfo p on s.UserId = p.UserId inner join address a on p.UserId = a.UserId inner join category c on s.CategoryId = c.CategoryId inner join subcategory sc on s.SubCategoryId = sc.SubCategoryId"
				+ " where CategoryName like ? OR SubCategoryName like ? OR FirstName LIKE ? OR LastName LIKE ?";
		try {
			query= java.net.URLDecoder.decode(query, "UTF-8");
			query = "%" + query + "%";
			System.out.print(query);
			java.sql.PreparedStatement searchPreparedStatement = helper.conn
					.prepareStatement(sqlquery);
			searchPreparedStatement.setString(1,query);
			searchPreparedStatement.setString(2,query);
			searchPreparedStatement.setString(3,query);
			searchPreparedStatement.setString(4,query);
	...
	@Produces(MediaType.TEXT_PLAIN)
	public String getAllReviewsForService(
			@PathParam("ServiceId") String ServiceId) {
		String result = "";
		String query = "select * from "
				+ "review r inner join personalinfo p on r.RaterUserId = p.UserId "
				+ "inner join service s on r.ServiceId = s.ServiceId "
				+ "inner join category c on s.CategoryId = c.CategoryId "
				+ "inner join subcategory sc on s.SubCategoryId = sc.SubCategoryId  where r.ServiceId=?";
		MySqlHelper helper = new MySqlHelper();
		try {
			java.sql.PreparedStatement Statement = helper.conn
					.prepareStatement(query);
			...
			while (rs.next()) {
				...
				review.setRating(Double.toString(rs.getDouble("Rating")));
				...
				Category c = new Category();
				c.setCategoryName(rs.getString("CategoryName"));
				review.setCategory(c);

				SubCategory sc = new SubCategory();
				sc.setSubCategoryName(rs.getString("SubCategoryName"));
				review.setSubcategory(sc);

				User user = new User();
				user.setUserId(rs.getInt("RaterUserId"));
				user.setFirstName(rs.getString("FirstName"));
				user.setLastName(rs.getString("LastName"));
				user.setInitials(rs.getString("FirstName").substring(0, 1)
						+ "," + rs.getString("LastName").substring(0, 1));
				review.setUser(user);
	...
	public String AddReview(String requestJson) {
		...
		try {
			...
			query = "insert into review(ServiceId,Rating,Title, Comment,RaterUserId,isAnonymous,Date) values(?,?,?,?,?,?,?)";
			java.sql.PreparedStatement Statement = helper.conn
					.prepareStatement(query);
			Statement.setInt(1,
					Integer.parseInt(request.getReview().getServiceId()));
			Statement.setFloat(2,
					Float.parseFloat(request.getReview().getRating()));
			Statement.setString(3, request.getReview().getTitle());
			Statement.setString(4, request.getReview().getComment());
			Statement.setInt(5,
					Integer.parseInt(request.getReview().getRaterUserId()));
			Statement.setInt(6,
					Integer.parseInt(request.getReview().getIsAnonymous()));
			Statement.setString(7, request.getReview().getDate());

			Statement.executeUpdate();

			// now update the average rating
			query = "update service set AvgRating = (select avg(rating) from review where serviceid = ?) where serviceid =?";
			...
			/*
			 * }else{ query =
			 * "insert into review(ServiceId,Rating,Title, Comment,RaterUserId,Date) values(?,?,?,?,?,?)"
			 * ; java.sql.PreparedStatement Statement = helper.conn
			 * .prepareStatement(query); Statement.setInt(1,
			 * Integer.parseInt(request.getReview().getServiceId()));
			 * Statement.setFloat(2,
			 * Float.parseFloat(request.getReview().getRating()));
			 * Statement.setString(3, request.getReview().getTitle());
			 ...
			 * Statement.setInt(5,
			 * Integer.parseInt(request.getReview().getRaterUserId()));
			 * Statement.setString(6, request.getReview().getDate());
			 * 
			 * Statement.executeUpdate();
			 * 
			 * }
	...
	public String RegisterService(String requestJson) {
		...
		try {
			...
			int pricePerHour = Integer.parseInt(request.getPricePerHour()
					.split(" ")[0]);
			int serviceId = insertService(helper, request.getUserId(),
					categoryId, subcategoryId, pricePerHour,
					Integer.parseInt(request.getWillingToTravelInMiles()),
					request.getAdvertise());
	...
	public String UpdateTutorInfo(String requestJson) {
		...
		MySqlHelper helper = new MySqlHelper();
		String query = "update (personalinfo p inner join login l on p.UserId = l.UserId )"
		...
		try {
			java.sql.PreparedStatement updateProfilePreparedStatement = helper.conn
					.prepareStatement(query);
			...
			updateProfilePreparedStatement.setString(3,
	...
	@GET
	@Produces(MediaType.TEXT_PLAIN)
	public String getNearestTutors(@PathParam("Lattitude") String lattitude,
			@PathParam("Longitude") String longitude) {
		String result = "";
		MySqlHelper helper = new MySqlHelper();
		String query = "SELECT addressid, ( 3959 * acos( cos( radians('"
				+ lattitude
				+ "') ) * cos( radians( lattitude ) ) * cos( radians( longitude ) - radians('"
				+ longitude
				+ "') ) + sin( radians('"
				+ lattitude
				+ "') ) * sin( radians( lattitude ) ) )) AS distance FROM address HAVING distance < 25 ORDER BY distance LIMIT 0 , 20";
		// String query =
		// "call getnearesttutor('"+lattitude+"','"+longitude+"')";
		try {
			System.out.println(query);
			...
			while (rs.next()) {
				...
				distanceLookUp.put(rs.getInt("addressid"),
			...
			while (rs.next()) {
				...
				service.setServiceName(rs.getString("SubCategoryName") + " by "
						+ rs.getString("LastName"));

				service.setMiles((double) distanceLookUp.get(rs
						.getInt("AddressId")));
	...
	@GET
	@Produces(MediaType.TEXT_PLAIN)
	public String getAllSubCategoriesForCategory(
		...
		try {
			java.sql.PreparedStatement getAllSubCategoriesPreparedStatement = helper.conn
					.prepareStatement(query);
			getAllSubCategoriesPreparedStatement.setInt(1,
			...
			while (rs.next()) {
				...
				category.setSubCategoryId(String.valueOf(rs
	...
	public int insertService(MySqlHelper helper, int UserId, int CategoryId,
			int SubCategoryId, int pricePerHour, int Distance,
			String isAdvertised) {
